...
from ..data import main_page, login_page, registration_page, forgot_password_page, cabinet_page
import pytest
...
class TestLogin:
    @pytest.mark.parametrize(
        'link, button',
        [
        (main_page(), ACCOUNT_BUTTON),
        (main_page(), ACCOUNT_LINK),
        (registration_page(), LOGIN_FROM_REGISTER_PAGE_LINK),
        (forgot_password_page(), LOGIN_FROM_FORGET_PASSWORD_PAGE_LINK)
        ],
        ids=["Main - Enter","Personal cabinet - Enter","Registration - Enter","Password reset - Enter"]
    )

    def test_login(self, driver, link, wait, button, login_data):
        self.driver = driver
